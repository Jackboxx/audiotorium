...
export const ThumbNailSchema =
    z.object({url : z.string().url(), width : z.number(), height : z.number()});
...
export const YouTubeVideoSchema = z.object({
  ...
  snippet : z.object({thumbnails : ThumbNailsSchema})
})

export const YouTubeSearchResultSchema =
    z.object({items : YouTubeVideoSchema.array()})
...
export async function searchForVideos(search: string): Promise<Response> {
  return await fetch(`https://www.googleapis.com/youtube/v3/search?q=${
      search}&part=snippet&type=video&maxResults=10&key=${
